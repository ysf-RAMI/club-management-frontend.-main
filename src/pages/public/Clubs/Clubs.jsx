...
import { fetchClubs, changeCategory, sortClubs, searchClubs, clearFilters } from '../../../app/clubSlice';
    ...
    <div>
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-violet-600 text-white p-12 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-4 bg-white p-7 px-22 shadow-md border-b-2 border-gray-300">
          ...
          new Set(
            clubs
              .map((c) => (c.categorie || c.category || '').trim())
              .filter(Boolean)
          )
            ).map((category, idx) => (
                  ...
                  return (
                    <div key={rowIndex} className="flex flex-col md:flex-row justify-center gap-6">
                      {rowClubs.map((club) => (
                        ...
                        >
                          <img
                            src={club.image || '/public/vite.svg'}
                            alt={club.name || 'Club image'}
                            className="w-full h-40 object-cover rounded-lg mb-4"
                          />
                          <div className="flex justify-between items-center mb-2">
                            <span
                              className={`text-xs font-semibold px-2 py-1 rounded-full ${(club.categorie || club.category || '').toLowerCase() === 'technologie' ||
                                (club.categorie || club.category || '').toLowerCase() === 'technology'
                                ? 'bg-blue-100 text-blue-800'
                                : (club.categorie || club.category || '').toLowerCase() === 'academic'
                                  ? 'bg-green-100 text-green-800'
                                  : (club.categorie || club.category || '').toLowerCase() === 'arts & culture' ||
                                    (club.categorie || club.category || '').toLowerCase() === 'art'
                                    ? 'bg-purple-100 text-purple-800'
                                    : (club.categorie || club.category || '').toLowerCase() === 'sports'
                                      ? 'bg-red-100 text-red-800'
                                      : (club.categorie || club.category || '').toLowerCase() === 'volunteer'
                          ...
                          <div className="grid grid-cols-2 gap-4 text-sm text-gray-600 mb-4">
                            ...
                            <div>
                              ...
                              <div>{club.created_at ? new Date(club.created_at).toLocaleString() : '—'}</div>
                            </div>
                          </div>
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <div className="text-xs text-gray-400">
                              Updated: {club.updated_at ? new Date(club.updated_at).toLocaleDateString() : '—'}
                            </div>
                          </div>
                        </div>
                      ))}
